...
// Adapted from A2560K OS/f bitmap.h/.c starting June 24, 2024
...
#define PARAM_ALLOCATE_VRAM		true	//!< for Bitmap_New
#define PARAM_USE_PASSED_VRAM	false	//!< for Bitmap_New
...
//	Font*			font_;		//!< the currently selected font. All text drawing activities will use this font face.
	uint8_t*		addr_;		//!< address of the start of the bitmap, within the machine's global address space. This is not the VICKY's local address for this bitmap. This address MUST be within the VRAM, however, it cannot be in non-VRAM memory space.
	...
	bool			allocated_;	//!< a way to know if this bitmap is pointing to RAM that we allocated, or that some other process manages
} Bitmap;
...
//! @param	new_alloc - whether or not new RAM needs to be allocated for this bitmap, or if the passed location should be used. If false,  no space will be allocated for the bitmap VRAM. If true, width * height area of memory will be allocated in standard memory.
//! @param	vram_loc - if new_alloc was false, this address will be set for the bitmap's VRAM location. Ignored if new_alloc was true. Use PARAM_ALLOCATE_VRAM and PARAM_USE_PASSED_VRAM
Bitmap* Bitmap_New(int16_t width, int16_t height, bool new_alloc, uint8_t* vram_loc);
...
//! @param	dst_x - the horizontal location within the destination bitmap to copy pixels to. May be negative.
//! @param	dst_y - the vertical location within the destination bitmap to copy pixels to. May be negative.
...
//! @param	src_bm - the source bitmap. It must have a valid address within the VRAM memory space.
//! @param	dst_bm - the destination bitmap. It must have a valid address within the VRAM memory space. It can be the same bitmap as the source.
//! @param	src_x - the left coordinate within the source bitmap, for the rectangle you want to copy. Must be non-negative.
//! @param	src_y - the upper coordinate within the source bitmap, for the rectangle you want to copy. Must be non-negative.
//! @param	dst_x - the left coordinate within the destination bitmap to copy pixels to. Must be non-negative.
//! @param	dst_y - the upper coordinate within the destination bitmap to copy pixels to. Must be non-negative.
//! @param	width - the width of the copy, in pixels.
//! @param	height - the height of the copy, in pixels.
...
//! @param	src_bm - the source bitmap. It must have a valid address within the VRAM memory space.
//! @param	dst_bm - the destination bitmap. It must have a valid address within the VRAM memory space. It can be the same bitmap as the source.
//! @param	src_x - the left coordinate within the source bitmap, for the tile you want to copy. Must be non-negative.
//! @param	src_y - the upper coordinate within the source bitmap, for the tile you want to copy. Must be non-negative.
//! @param	width - the width of the tile to be derived from the source bitmap, in pixels.
//! @param	height - the height of the tile to be derived from the source bitmap, in pixels.
