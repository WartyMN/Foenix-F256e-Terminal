...
// in addition to extracting debug stuff from general.c, file routines changed to have conditionals for FatFS library not C library
...
//   able to pass format string and multiple variables when needed
	...
	void General_LogError(const char* format, ...)
	{
		...
		the_len = strlen(debug_out_buffer);
			...
			debug_ffs_result = f_write(&debug_log_file_handle, debug_out_buffer, the_len, &debug_bytes_written_to_disk);
				// comment: if this fails, it's only for debugging, not going to worry about it.
		#endif
	}
#endif
	...
	void General_LogAlloc(const char* format, ...)
	{
		...
		#else
			//write(debug_log_file_handle, debug_out_buffer, the_len);
			//write(debug_log_file_handle, "started log file", 16);
			debug_ffs_result = f_write(&debug_log_file_handle, debug_out_buffer, the_len, &debug_bytes_written_to_disk);
				// comment: if this fails, it's only for debugging, not going to worry about it.
		#endif
	}
...
// 		Serial_InitUART();
		...
		//debug_log_file_handle = open(the_file_path, O_WRONLY);
		debug_ffs_result = f_open(&debug_log_file_handle, the_file_path, FA_WRITE | FA_CREATE_ALWAYS);
		
		//if (debug_log_file_handle < 1)
		//if (debug_log_file == NULL)
		if (debug_ffs_result != FR_OK)
		{
		...
		//write(debug_log_file_handle, "started log file", 16);
		debug_ffs_result = f_write(&debug_log_file_handle, "started log file", 16, &debug_bytes_written_to_disk);
	
		if (debug_ffs_result != FR_OK || debug_bytes_written_to_disk < write_len)
		{
			printf("General_LogInitialize: log file could not be written to! \n");
...
void General_LogCleanUp(void)
{
	#if defined USE_SERIAL_LOGGING
	#else
		//if (debug_log_file_handle > 0)
		//if (debug_log_file != NULL)
		if (debug_log_file_handle.err == 0)
		{
			f_close(&the_target_handle);
		}
	#endif
