...
uint8_t* Bitmap_GetMemLocForXY(Bitmap* the_bitmap, int16_t x, int16_t y);
...
Bitmap* Bitmap_New(int16_t width, int16_t height, bool in_vram)
{
	Bitmap*		the_bitmap;
	...
	{
		the_bitmap->addr_int_ = (uint32_t)VRAM_START_ADDR;
		the_bitmap->addr_ = (uint8_t*)the_bitmap->addr_int_;		
	}
...
// MB: we're not allocating in F256, so freeing would be a BAD idea
// 	if ((*the_bitmap)->addr_)
// 	{
...
bool Bitmap_Blit(Bitmap* src_bm, int16_t src_x, int16_t src_y, Bitmap* dst_bm, int16_t dst_x, int16_t dst_y, int16_t width, int16_t height)
{
	...
	uint32_t		copy_size;
	int16_t			j;
	uint16_t		src_skiplen;
	uint16_t		dst_skiplen;
	...
	src_skiplen = src_bm->width_;
	dst_skiplen = dst_bm->width_;
	
	#if defined _DMAC_
		// Take advantage of Vicky's DMA engine, using a C routine
		DMA_CopyRect(the_write_loc, dst_bm->width_, the_read_loc, width, height, src_bm->width_);

	#elif defined _DMAA_
		// Take advantage of Vicky's DMA engine, using an ASM routine

		// set up the 20-bit address for to/from
		R16(ZP_TO_ADDR) = the_write_loc_int & 0xFFFF;
		R8(ZP_TO_ADDR + 2) = (the_write_loc_int >> 16) & 0xFF;

		// set up the 20-bit address for to/from
		R16(ZP_FROM_ADDR) = the_read_loc_int & 0xFFFF;
		R8(ZP_FROM_ADDR + 2) = (the_read_loc_int >> 16) & 0xFF;

		if (width % 2) width--;	
		*(uint16_t*)ZP_WIDTH_LO = width;
		*(uint16_t*)ZP_HEIGHT_LO = height;
		*(uint16_t*)ZP_SRC_STRIDE_LO = src_bm->width_;
		*(uint16_t*)ZP_DST_STRIDE_LO = dst_bm->width_;
		
		Memory_CopyRectWithDMA();
	
	#else
		// skip the DMA engine, just use pointers
		...
		for (j = 0; j < height; j++)
		{
			...
			the_write_loc += dst_skiplen;
			the_read_loc += src_skiplen;
		}

		//  // Note: this below does weird stuff, as of 2024/07/02. do not use. 
		//	int16_t			i;
		// 	for (j = 0; j < height; j++)
		// 	{
...
// 	the_write_loc = (uint8_t*)the_write_loc_int;
	the_write_loc = the_bitmap->addr_;
	the_write_len = (uint32_t)the_bitmap->width_ * (uint32_t)the_bitmap->height_;

	#if defined _DMAC_
		// Take advantage of Vicky's DMA engine, using a C routine
		DMA_Fill(the_write_loc, the_write_len, the_color);
		//DMA_FillRect((uint8_t*)VRAM_START_ADDR, 320, 320, 240, 85);
		...
		for (the_write_loc=(uint8_t*)0x50000, i=0; i < 11328; i++)
		{
			*the_write_loc++ = the_color;
		}

		// below only clears make 10k of pixels. not sure what's going. maybe a calypsi data model thing? 
		//memset(the_bitmap->addr_, the_color, the_write_len);
	#endif
...
// 	DEBUG_OUT(("%s %d: width=%i, write_len=%lu (unsigned), write_len=%li", __func__, __LINE__, width, write_len, write_len));
	...
	#if defined _DMAC_
		// Take advantage of Vicky's DMA engine, using a C routine
		DMA_FillRect((uint8_t*)the_write_loc_int, fat_bmap_width, width, height, short_color);
