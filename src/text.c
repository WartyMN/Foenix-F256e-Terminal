...
bool Text_FillMemory(bool for_attr, uint8_t the_fill)
{
	...
	if (for_attr)
	{
		the_write_loc = (uint8_t __attribute__((far)) * )VICKY_TEXT_ATTR_RAM;
	}
	else
	{
		the_write_loc = (uint8_t __attribute__((far)) * )VICKY_TEXT_CHAR_RAM;
	}
...
bool Text_FillMemoryBoxBoth(uint8_t x, uint8_t y, uint8_t width, uint8_t height, uint8_t the_char, uint8_t the_attribute_value)
{
	...
	for (; y <= max_row; y++)
	{
		...
		the_char_loc += SCREEN_NUM_COLS;
		the_attr_loc += SCREEN_NUM_COLS;
	}
...
bool Text_FillBox(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, uint8_t the_char, uint8_t fore_color, uint8_t back_color)
{
	...
	// same for d1, as we account for that in the next function called
	dx = x2 - x1 + 1;
	dy = y2 - y1 + 1;
...
bool Text_InvertBox(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2)
{
	...
	// get VRAM address as 32 bit integer and convert to 24 bit pointer
	Text_SetXY(x1,y1);
	the_write_loc = text_char_addr + VICKY_TEXT_RAM_DELTA;
	...
	for (; y1 <= y2; y1++)
	{
		for (the_col = x1; the_col <= x2; the_col++)
		{
			...
			//fore_nibble = ((the_attribute_value2 & 0x0F) << 4);
			//^^ above is fine in cc65, but doesn't produce right results in Calypsi. Splitting it out fixes it.
			back_nibble = the_attribute_value & 0xF0;
			back_nibble = back_nibble >> 4;
			fore_nibble = the_attribute_value & 0x0F;
			fore_nibble = fore_nibble >> 4;
...
void Text_SetXY(uint8_t x, uint8_t y)
{
	...
	the_vram_plot = (uint32_t)((SCREEN_NUM_COLS * y) + x);
	the_vram_plot += VICKY_TEXT_CHAR_RAM;
	
	// save the new current address, x, y position, and also tell VICKY where the cursor should be
	text_char_addr = (uint8_t*)(the_vram_plot); 
	text_x = x;
	text_y = y;
	R8(VICKY_TEXT_X_POS) = text_x;
	R8(VICKY_TEXT_Y_POS) = text_y;
...
bool Text_DrawString(char* the_string, uint8_t fore_color, uint8_t back_color)
{
	...
	// draw the attributes
	memset(the_attr_loc, the_attribute_value, the_len);
...
// 	avail_height = the_dialog_template->height_ - 7;
	...
	for (; i >= 0; i--)
	{
		...
		btn_x -= btn_width[i];
		Text_DrawStringAtXY(btn_x, btn_y, the_dialog_template->btn_label_[i], btn_color, APP_BACKGROUND_COLOR);
	}
	...
	do
	{
		...
		if (player_input == CH_ESC || player_input == KEY_BREAK)
		{
			break;
		}
...
bool Text_GetStringFromUser(char* the_buffer, int8_t the_max_length, uint8_t start_x, uint8_t start_y, bool overwrite_mode)
{
	...
	while ( (the_char = Keyboard_GetChar() ) != CH_ENTER)
	{
		...
		else if (the_char == KEY_CURS_DOWN)
		{
		...
		else if (the_char == KEY_CURS_RIGHT)
		{
			if (curr_pos < curr_len)
			{
		...
		else if (the_char == KEY_CURS_LEFT)
		{
			if (curr_pos > 0)
			{
