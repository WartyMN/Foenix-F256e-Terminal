...
void General_DelayTicks(uint16_t ticks)
{
	uint16_t	i;
	uint16_t	j;
	uint16_t	fool_the_compiler;
	
	for (i = 0; i < ticks; i++)
	{
		for (j = 0; j < 5; j++)
		{
			fool_the_compiler = j; // just trying to do something so that compiler won't optimize all this code away.
		}
	}
}
